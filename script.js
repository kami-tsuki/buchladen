    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            databases.forEach(function (database) {
                $('#sidebar').append('<a id="action-database-' + database + '" href="#' + database + '" class="list-group-item list-group-item-action">' + database + '</a>');
            });
            $('#loading-banner').hide();
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            tables.forEach(function (table) {
                $('#topbar').append('<a id="action-table-' + database + '-' + table + '" href="#' + database + '-' + table + '" class="list-group-item list-group-item-action">' + table + '</a>');
            });
            $('#loading-banner').hide();
        },
        error: function (response) {
...
function getTableData(database, table) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#nav-tabContent-table').append('<button id="addRowButton" class="btn btn-primary">Add Row</button>');
            $('#addRowForm').empty();
            data.columns.forEach(column => {
                if (column !== 'id') {
                    $('#addRowForm').append(`<label for="${column}">${column}</label><input type="text" id="${column}" name="${column}" class="form-control">`);
                }
            });
            $('#loading-banner').hide();
        },
...
function sortTable(table, colIndex) {
    ...
    rows.sort((rowA, rowB) => direction * (rowA.cells[colIndex].innerText > rowB.cells[colIndex].innerText ? 1 : -1));
    table.innerHTML = '';
    table.append(header);
    table.append(...rows);
...
function filterTable(table, query) {
    ...
    let header = rows.shift();
    for (let row of rows) {
        let shouldHide = Array.from(row.cells).every(cell => !cell.innerText.includes(query));
...
$('#saveRow').click(function () {
    ...
    let data = $('#addRowForm').serializeArray().reduce(function (obj, item) {
